...
class Contact extends Component {
  ...
  validate(firstname, lastname, telnum, email) {
    ...
    const reg = /^\d+$/; // This is an expression which says that string of all characters should be numbers
    if (this.state.touched.telnum && !reg.test(telnum)) {
      errors.telnum = "Telephone numbers should contain only numbers";
    }
    var checkA = email.split("").filter((x) => x === "@");
    console.log(checkA);
    if (this.state.touched.email && checkA.length !== 1) {
